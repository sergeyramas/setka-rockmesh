...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    // Mobile / reduced-motion fallback: autoplay loop, tooltips fully visible.
    ...
    // Desktop scroll-scrubbed setup. Wait for video metadata so duration is known.
      ...
      scrollTrigger = ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          if (video.duration) {
            ...
            // Avoid micro-updates that thrash the decoder.
            if (Math.abs(video.currentTime - target) > 0.02) {
              video.currentTime = target
            }
          }

          // 2. Text fade-out over the first 30% of progress.
          if (textEl) {
          ...
          // 3. Tooltip reveals — each over an 8% window starting at appearAt.
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen overflow-hidden bg-white">
        ...
        {/* Dark gradient on the left for text legibility */}
        <div
          aria-hidden
        ...
        {/* Tooltips on the disassembling mesh */}
        {TOOLTIPS.map((t, i) => (
          <div
            key={t.label}
            ref={(el) => {
